#!/usr/bin/env python3
"""
Generate golden oscillator square output CSV for one cycle at a given
frequency and sample rate, by reading the golden Waveform square CSV at the
adjusted phase increment with linear interpolation.

Mirrors C++ Oscillator: phase_increment = freq * waveform_size / sample_rate.

Usage:
    python TESTS/OSCILLATOR/GOLDEN/generate_golden_oscillator_square.py --freq 441 --sample-rate 44100
"""
...
WAVEFORM_CSV = SCRIPT_DIR.parent.parent / "WAVEFORM" / "GOLDEN" / "SQUARE" / "GOLDEN_SQUARE_8096.csv"
OUTPUT_DIR   = SCRIPT_DIR / "SQUARE"
...
def generate(freq: float, sample_rate: float) -> Path:
    ...
    out_path = OUTPUT_DIR / f"GOLDEN_OSC_SQUARE_{freq_tag}Hz_{sr_tag}SR.csv"

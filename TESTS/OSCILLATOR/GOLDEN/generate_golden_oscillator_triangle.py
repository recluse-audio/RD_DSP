#!/usr/bin/env python3
"""
Generate golden oscillator triangle output CSV for one cycle at a given
frequency and sample rate, by reading the golden Waveform triangle CSV at the
adjusted phase increment with linear interpolation.

Mirrors C++ Oscillator: phase_increment = freq * waveform_size / sample_rate.

The triangle waveform table is itself band-limited (mellow, fixed K harmonics)
so playback at typical audio frequencies has minimal aliasing.
...
Usage:
    python TESTS/OSCILLATOR/GOLDEN/generate_golden_oscillator_triangle.py --freq 441 --sample-rate 44100
"""
...
WAVEFORM_CSV   = SCRIPT_DIR.parent.parent / "WAVEFORM" / "GOLDEN" / "TRIANGLE" / "GOLDEN_TRIANGLE_8096.csv"
OUTPUT_DIR     = SCRIPT_DIR / "TRIANGLE"
...
def generate(freq: float, sample_rate: float) -> Path:
    ...
    out_path = OUTPUT_DIR / f"GOLDEN_OSC_TRIANGLE_{freq_tag}Hz_{sr_tag}SR.csv"
